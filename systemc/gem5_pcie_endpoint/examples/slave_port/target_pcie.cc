...
TargetPCIe::TargetPCIe(sc_module_name name) : sc_module(name),
    target_socket("target_socket"),
    initiator_socket("initiator_socket"),
    peq(this, &TargetPCIe::peq_cb),
    reconstructed_payload(nullptr),
    reconstructed_length(0)
{
    target_socket.register_nb_transport_fw(this, &TargetPCIe::nb_transport_fw);
    initiator_socket.register_nb_transport_bw(this, &TargetPCIe::nb_transport_bw);
}

tlm_sync_enum TargetPCIe::nb_transport_fw(tlm_generic_payload& trans, tlm_phase& phase, sc_time& t) {
    peq.notify(trans, phase, t);
    return TLM_ACCEPTED;
}

tlm_sync_enum TargetPCIe::nb_transport_bw(tlm_generic_payload& trans, tlm_phase& phase, sc_time& t) {
    peq.notify(trans, phase, t);
    return TLM_ACCEPTED;
}

void TargetPCIe::peq_cb(tlm_generic_payload& trans, const tlm_phase& phase) {
    if (phase == BEGIN_REQ) { // Request from Host PCIe
