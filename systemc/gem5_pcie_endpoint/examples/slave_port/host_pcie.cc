...
HostPCIe::HostPCIe(sc_module_name name) : sc_module(name),
    target_socket("target_socket"),
    initiator_socket("initiator_socket"),
    peq(this, &HostPCIe::peq_cb),
    tx_delay(10, SC_NS),
    next_flit_id(1)
{
    target_socket.register_nb_transport_fw(this, &HostPCIe::nb_transport_fw);
    initiator_socket.register_nb_transport_bw(this, &HostPCIe::nb_transport_bw);
...
tlm_sync_enum HostPCIe::nb_transport_fw(tlm_generic_payload& trans, tlm_phase& phase, sc_time& t) {
    peq.notify(trans, phase, t);
    return TLM_ACCEPTED;
}

tlm_sync_enum HostPCIe::nb_transport_bw(tlm_generic_payload& trans, tlm_phase& phase, sc_time& t) {
    peq.notify(trans, phase, t);
    return TLM_ACCEPTED;
}

void HostPCIe::peq_cb(tlm_generic_payload& trans, const tlm_phase& phase) {
    if (phase == BEGIN_REQ) { // Reqeust from Bridge
        ...
        // Handle responses from Target PCIe (Acks/Naks)
        PCIeFlitExtension* ext;
